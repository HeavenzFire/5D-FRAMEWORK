...
def on_message(client, userdata, message):
    ...
    iot_data[topic] = payload

mqtt_client = mqtt.Client()
mqtt_client.on_message = on_message
mqtt_client.connect("your_mqtt_broker_url")
...
def generate_response(user_input, data):
    combined_input = f"User input: {user_input} | Data: {data} | IoT Data: {iot_data}"
    response = generator(combined_input, max_length=50, num_return_sequences=1)
    return response[0]['generated_text']

def spirit_engine():
    print("Welcome to Spirit with IoT integration. How can I assist you today?")
